...
mobyLoc = 'C:\Users\Eren Can Ileri\Desktop\Parse\MobyDick.txt'; % Change this filepath to wherever 'MobyDick.txt' is
...
newLine = fgetl(moby); % get first line
...
while ischar(newLine) %fill in condition here
...
% -------------------- % 
    ...
    if %fill in condition here
        words = strsplit(newLine)% split line into words at whitespace
        
    % -------------------- % 
    
        % For each word within the current line, you'll need to do a couple
        % things: 
        % (1) Remove all whitespaces, and convert all of the letters to the
